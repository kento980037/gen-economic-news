...
class ScriptGenerator:
    ...
    def generate_script(
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=messages,
                max_tokens=4096,
                temperature=0.9,  # カジュアルな表現を増やすため高めに設定
            )
    ...
    def _build_prompt(
        ...
        """単一記事用のプロンプトを構築"""
        prompt = f"""以下の金融ニュース記事を元に、YouTuberのような超カジュアルな話し言葉で台本を作成してください。

【絶対に守るべきルール】
1. 「〜なんですよ」「〜なんですけど」「〜じゃないですか」を各文に必ず入れる
2. 「まあ」「で」「あの」「えーと」「っていう」などのフィラーを頻繁に使う
3. 「ヤバい」「マジで」「結構」などの若者言葉を自然に混ぜる
4. 「です。ます。」の連続は絶対NG。必ず「〜なんですよ。」「〜なんですけど。」に変える

【NGな書き方 vs OKな書き方】
❌ NG: 「今日はアジアのテクノロジー株が急落したニュースについてお話しします」
✅ OK: 「どうも！今日なんですけど、アジアのテック株がマジでヤバいことになってまして」

❌ NG: 「結論から言うと、これがAIや半導体株のラリーのピークに近づいているかもしれないということなんですよ」
✅ OK: 「で、結論から言っちゃうとですね、AIとか半導体の株、そろそろピークかもしれないんですよ、これ」

❌ NG: 「投資家の皆さんが今後の相場にどう影響するか、3つのポイントで解説しますね」
✅ OK: 「まあ、今後どうなるか気になるじゃないですか。3つのポイントで話していきますね」

❌ NG: 「具体的には、例えば中国の主要テクノロジー企業の株価が軒並み影響を受けて」
✅ OK: 「で、具体的に言うとですね、中国のテック企業の株がもうバタバタ落ちてまして」

❌ NG: 「これ、結構ヤバいですよね」（これは良い！こういう感じで全体を書く）
✅ OK: 「これマジでヤバいんですよ」

❌ NG: 「投資家の心理に影響を与えている状況です」
✅ OK: 「投資家の人たち、結構焦ってる感じなんですよね」
...
- 目標時間: 2〜3分
- スタイル: カジュアルな話し言葉（{self.style}形式）
- トーン: {self.tone}（フレンドリーで親しみやすく）
...
✅ **5. 専門用語は必ずカジュアルに補足説明する（超重要）**
- 専門用語を使ったら、直後に必ずカジュアルに説明を入れる

【カジュアルな補足説明のテンプレート】
- 「QE、まあ量的緩和っていうやつなんですけど、要するに中央銀行がお金をジャンジャン市場に流すってことなんですよ」
- 「テーパリング、つまり金融緩和を少しずつ減らしていくってやつですね」
- 「FRB、アメリカの中央銀行なんですけど」
- 「インフレ率、まあ物の値段がどれくらい上がってるかってやつですね」
- 「利回り、要するに投資したお金がどれだけ増えるかって話なんですけど」
- 「ドルインデックス、ドルがどれくらい強いかを示す指標なんですよ」
...
✅ **6. カジュアルに感情を混ぜる（冷静+少しの驚き）**
台本に自然な感情表現を入れる：
- 「これ、正直かなり意外なんですよ」
- 「投資家にとっては結構いいニュースですね」
- 「この数字、ちょっと注目した方がいいと思うんですよね」
- 「まあ、これはちょっとヤバいかもしれないです」
※ただし信頼性を損なわない程度に
...
✅ **10. カジュアルな話し言葉で書く（最重要）**
- 「〜なんですよ」「〜なんですけど」「〜じゃないですか」を頻繁に使う
- 「まあ」「で」「あの」「えーと」などのフィラーを自然に入れる
- 短い文で区切る
- 友達に話すような親しみやすい口調
...
1. オープニング（約15秒分）
   ❌ NG: 「こんにちは！今日は〜についてお話しします」
   ✅ OK: 「どうも！今日なんですけど、〜っていうヤバいニュースがあって」
   ✅ OK: 「はい、というわけでですね、今回〜っていう話なんですけど」
   - 必ず「どうも！」「はい！」などのカジュアルな挨拶で始める
   - 結論を先に言う（「マジでヤバい」「結構意外」などの感想を入れる）
