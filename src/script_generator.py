...
class ScriptGenerator:
    ...
    def _build_prompt(
        ...
        """単一記事用のプロンプトを構築"""
        prompt = f"""以下の金融ニュース記事を元に、カジュアルな話し言葉でYouTube金融ポッドキャスト用の台本を作成してください。

【超重要】
- 「〜なんですよ」「〜なんですけど」「〜じゃないですか」などのカジュアルな語尾を多用すること
- 「まあ」「で」「あの」「えーと」などの自然なフィラーを入れること
- 友達に話すような親しみやすい口調で書くこと
- 堅苦しい「です・ます」の連続は絶対NG
