...
⚠️ **超重要**: 9分の動画を作るため、各セクションを十分に長く書いてください

1. オープニング（約20秒分 = 100文字）
   - 結論先出し
   - 今日の価値を明示
   - 自然な導入で始める

2. ポイント1（約120-150秒分 = 600-750文字）
   ⚠️ 必ず600文字以上書く
   - 1つ目の重要ポイントを深掘り
   - 具体的な数字、企業名、事例を豊富に（最低3つ以上）
   - 過去のデータとの詳細な比較（具体的な数字で）
   - 専門家の見方や市場の反応（複数の専門家・機関を引用）
   - なぜそうなったのか、背景を深く掘り下げる
   - 段落で区切る（改行2つ）

3. ポイント2（約120-150秒分 = 600-750文字）
   ⚠️ 必ず600文字以上書く
   - 2つ目の重要ポイントを深掘り
   - なぜそうなったのか背景を詳しく（複数の要因）
   - 複数の要因を丁寧に説明（最低3つ以上）
   - 関連する過去の事例を詳しく紹介
   - 市場への影響を具体的な数字で説明
   - 段落で区切る（改行2つ）

4. ポイント3（約120-150秒分 = 600-750文字）
   ⚠️ 必ず600文字以上書く
   - 3つ目の重要ポイントを深掘り
   - 追加の背景や関連情報を深掘り
   - 他国・他市場との比較（具体的なデータで）
...
5. 今後の展開（約90-120秒分 = 450-600文字）
   ⚠️ 必ず450文字以上書く
   - 短期的な予測を詳しく（1-3ヶ月）
   - 中期的な見通し（6ヶ月-1年）
   - 長期的な展望（1年以上）
   - 投資家が取るべき具体的な行動（複数のシナリオで）
   - 類似ケースとの比較
   - 段落で区切る（改行2つ）
...
⚠️ **文字数チェック**:
- ポイント1-3: 各600文字以上（合計1800文字以上）
- その他のセクション: 合計約900文字
- 総合計: 2700文字前後を必ず達成してください
...
⚠️⚠️⚠️ **最重要・最終確認** ⚠️⚠️⚠️
出力前に必ず以下を確認してください：
1. ポイント1の文字数は600文字以上ありますか？
2. ポイント2の文字数は600文字以上ありますか？
3. ポイント3の文字数は600文字以上ありますか？
4. 台本全体で2700文字前後ありますか？

もし足りない場合は、各ポイントに以下を追加してください：
- より詳しい背景説明
- 具体的な過去事例（年月日と数字付き）
- 複数の専門家・機関の見解
- 他国・他市場との詳細な比較
...
⚠️ 上記の例文は短縮版です。実際の台本では各ポイントを2-3倍に拡張してください。
"""
