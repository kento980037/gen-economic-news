...
"""
        ...
        # 3. ポイント1（1400文字）
        logger.info("Generating point 1 (1400 chars)...")
        sections['point1'] = self._generate_section(
            "ポイント1",
            article_info,
            1400,
            "最も重要なポイント。具体的な数字、企業名、事例を豊富に使い、背景や原因を深掘りする。"
        )

        # 4. ポイント2（1400文字）
        logger.info("Generating point 2 (1400 chars)...")
        sections['point2'] = self._generate_section(
            "ポイント2",
            article_info,
            1400,
            "2番目に重要なポイント。過去事例との比較、専門家の見解、市場への影響を詳しく解説。"
        )

        # 5. ポイント3（1200文字）
        logger.info("Generating point 3 (1200 chars)...")
        sections['point3'] = self._generate_section(
            "ポイント3",
            article_info,
            1200,
            "追加の重要ポイントまたは関連情報。他国・他市場との比較、投資家への具体的影響。"
        )
        ...
        keywords = self._generate_keywords(article_info, full_script)

        script_length = len(full_script)
        logger.info(f"✓ Script generated by sections. Total length: {script_length} chars (target: {target_chars})")
