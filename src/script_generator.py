...
"""
    ...
    def _edit_script(self, script: str, target_chars: int) -> str:
        ...
        Returns:
            添削後の台本
        """
        prompt = f"""以下の台本を添削してください。各セクションを個別に生成したため、重複や冗長性があります。

【現在の台本】（{len(script)}文字）
{script}

【添削要件】
1. **重複削除**: 同じ内容が複数回出てくる場合は1回にまとめる
   - 特に数字（43億ドル、1023億ドルなど）や事実の重複
2. **流れの改善**: セクション間の繋がりを自然にする
   - 「さて、今日は〜」「さて、ここからは〜」などの不自然な接続詞を削除または改善
3. **冗長性削除**: 同じ意味の文を異なる表現で繰り返している箇所を簡潔にする
4. **専門用語の説明**: 説明が不足している専門用語があれば追加
5. **文字数維持**: 可能な限り{target_chars}文字前後を維持（最低でも{int(target_chars * 0.8)}文字は確保）

【重要】
- 情報の削除ではなく、重複の削除が目的
- 具体的な数字、企業名、日付は保持
- 話し言葉の自然さを維持
- 構成（オープニング→ポイント→まとめ→CTA）は維持
